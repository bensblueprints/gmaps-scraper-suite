...
from prospecthunter.industries import INDUSTRIES
...
LICENSE_FILE = Path.home() / "AppData" / "Local" / "ProspectHunter" / "license.txt"
...
def _validate_license(key: str) -> tuple:
    ...
    try:
        ...
        r = requests.post(
            ...
            json={"license_key": key, "product": "prospecthunter", "machine_id": MACHINE_ID},
            timeout=5,
...
def _apply_treeview_style():
    ...
    style.configure("Treeview",
        background="#180E22", foreground="#C8BFD4",
        fieldbackground="#180E22", rowheight=26,
        font=("Consolas", 10))
    style.configure("Treeview.Heading",
        background="#1E1628", foreground="#9B59B6",
        relief="flat", font=("Consolas", 10, "bold"))
    style.map("Treeview",
        background=[("selected", "#3D1A5C")],
        foreground=[("selected", "#FFFFFF")])
    style.map("Treeview.Heading",
        background=[("active", "#1E1628")])


class LicenseBar(ctk.CTkFrame):
    def __init__(self, parent, on_activate: callable, **kw):
        super().__init__(parent, height=44, fg_color="#0D0A14", corner_radius=0, **kw)
        self.grid_propagate(False)
        self.on_activate = on_activate
        self._licensed = False

        ctk.CTkLabel(self, text="PROSPECTHUNTER",
                      font=ctk.CTkFont(size=15, weight="bold"),
                      text_color="#9B59B6").pack(side="left", padx=12)
        ...
        self.key_entry = ctk.CTkEntry(
            self, placeholder_text="License key (e.g. PH-XXXX-XXXX)",
...
class LeadsTable(ctk.CTkFrame):
    def __init__(self, parent, **kw):
        super().__init__(parent, fg_color="#0D0A14", corner_radius=0, **kw)
        self.grid_rowconfigure(1, weight=1)
        self.grid_columnconfigure(0, weight=1)

        # Search bar
        search_bar = ctk.CTkFrame(self, fg_color="#150F1E", height=36, corner_radius=0)
        ...
        self.lead_count_lbl = ctk.CTkLabel(
            ...
            font=ctk.CTkFont(size=11, weight="bold"), text_color="#9B59B6")
        ...
        tree_frame = tk.Frame(self, bg="#180E22")
...
class LeftPanel(ctk.CTkFrame):
    def __init__(self, parent, on_start, on_stop, **kw):
        super().__init__(parent, width=270, corner_radius=0, fg_color="#150F1E", **kw)
        self.grid_propagate(False)
    ...
    def _build_config_section(self):
        ...
        def _row(parent, label, default, row):
            ...
            ctk.CTkLabel(f, text=label, width=72, anchor="w",
                          text_color="#C8BFD4").pack(side="left")
        ...
        ctk.CTkCheckBox(cfg, text="Extract Emails + Platform",
                         ...
                         checkmark_color="#8E44AD",
                         border_color="#8E44AD").pack(anchor="w", padx=10, pady=2)
        ctk.CTkCheckBox(cfg, text="Fast mode (~20 results/query)",
                         variable=self.platform_var,
                         checkmark_color="#8E44AD",
                         border_color="#8E44AD").pack(anchor="w", padx=10, pady=(2, 10))
...
class LogPanel(ctk.CTkFrame):
    def __init__(self, parent, **kw):
        ...
        self.box = ctk.CTkTextbox(
            ...
            fg_color="#0D0A14", text_color="#C8BFD4",
...
class App(ctk.CTk):
    def __init__(self):
        ...
        right = ctk.CTkFrame(self, fg_color="#0D0A14", corner_radius=0)
        ...
        sep = ctk.CTkFrame(right, height=1, fg_color="#3D1A5C")
        ...
        ctk.CTkLabel(status_frame,
                      ...
                      font=ctk.CTkFont(family="Consolas", size=10),
                      text_color="#7B6F8C").pack(side="left", padx=8)
    ...
    def _start(self):
        ...
        self.status_lbl.configure(text="● RUNNING", text_color="#8E44AD")
    ...
    def _done(self):
        ...
        self.status_lbl.configure(text="● READY", text_color="#8E44AD")
        self.table.refresh_from_db()
        self._log("[SYSTEM] All jobs finished.")

    def _stop(self):
        self.engine.stop()
        self.status_lbl.configure(text="● STOPPING...", text_color="#7D3C98")
