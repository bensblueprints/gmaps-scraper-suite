"""
Leads.Baby — desktop application
...
from leadsbaby.industries import INDUSTRIES
...
LICENSE_FILE = Path.home() / "AppData" / "Local" / "LeadsBaby" / "license.txt"
LICENSE_FILE.parent.mkdir(parents=True, exist_ok=True)

MACHINE_ID = get_machine_id()

# Brand colors — hot pink + coral
BG_COLOR      = "#0D0608"
PANEL_COLOR   = "#1A0A10"
ACCENT        = "#FF6B9D"
ACCENT_HOVER  = "#E05585"
...
def _validate_license(key: str) -> tuple:
    ...
    try:
        ...
        r = requests.post(
            ...
            json={"license_key": key, "product": "leadsbaby", "machine_id": MACHINE_ID},
            timeout=5,
...
def _apply_treeview_style():
    ...
    style.configure("Treeview",
        ...
        fieldbackground=TREEVIEW_BG, rowheight=26,
        font=("Consolas", 10))
    style.configure("Treeview.Heading",
        background="#2A0A18", foreground=ACCENT,
        relief="flat", font=("Consolas", 10, "bold"))
    style.map("Treeview",
        background=[("selected", "#5C1A35")],
        foreground=[("selected", "#FFFFFF")])
    style.map("Treeview.Heading",
        background=[("active", "#2A0A18")])


class LicenseBar(ctk.CTkFrame):
    def __init__(self, parent, on_activate: callable, **kw):
        super().__init__(parent, height=44, fg_color=BG_COLOR, corner_radius=0, **kw)
        self.grid_propagate(False)
        self.on_activate = on_activate
        self._licensed = False

        ctk.CTkLabel(self, text="LEADS.BABY",
        ...
        self.key_entry = ctk.CTkEntry(
            self, placeholder_text="License key (e.g. LB-XXXX-XXXX)",
...
class LeadsTable(ctk.CTkFrame):
    def __init__(self, parent, **kw):
        super().__init__(parent, fg_color=BG_COLOR, corner_radius=0, **kw)
        self.grid_rowconfigure(1, weight=1)
        self.grid_columnconfigure(0, weight=1)

        search_bar = ctk.CTkFrame(self, fg_color=PANEL_COLOR, height=36, corner_radius=0)
        ...
        tree_frame = tk.Frame(self, bg=TREEVIEW_BG)
...
class LeftPanel(ctk.CTkFrame):
    def __init__(self, parent, on_start, on_stop, **kw):
        super().__init__(parent, width=270, corner_radius=0, fg_color=PANEL_COLOR, **kw)
        self.grid_propagate(False)
    ...
    def _build_industry_section(self):
        ...
        scroll = ctk.CTkScrollableFrame(self, fg_color="#200A14", label_text="")
    ...
    def _build_config_section(self):
        cfg = ctk.CTkFrame(self, fg_color="#200A14")
        ...
        def _row(parent, label, default, row):
            ...
            ctk.CTkLabel(f, text=label, width=72, anchor="w",
                          text_color=TREEVIEW_FG).pack(side="left")
        ...
        ctk.CTkLabel(cfg, text="LOCATION",
                      font=ctk.CTkFont(size=11, weight="bold"),
                      text_color="#7F8C8D").pack(anchor="w", padx=10, pady=(10, 2))
        ...
        ctk.CTkLabel(cfg, text="OPTIONS",
                      font=ctk.CTkFont(size=11, weight="bold"),
                      text_color="#7F8C8D").pack(anchor="w", padx=10, pady=(4, 2))
        ...
        ctk.CTkCheckBox(cfg, text="Extract Emails + Platform",
                         ...
                         border_color=ACCENT).pack(anchor="w", padx=10, pady=2)
        ctk.CTkCheckBox(cfg, text="Fast mode (~20 results/query)",
                         variable=self.platform_var,
                         checkmark_color=ACCENT,
                         border_color=ACCENT).pack(anchor="w", padx=10, pady=(2, 10))
...
class LogPanel(ctk.CTkFrame):
    def __init__(self, parent, **kw):
        ...
        self.box = ctk.CTkTextbox(
            ...
            fg_color=BG_COLOR, text_color=TREEVIEW_FG,
...
class App(ctk.CTk):
    def __init__(self):
        ...
        right = ctk.CTkFrame(self, fg_color=BG_COLOR, corner_radius=0)
        ...
        sep = ctk.CTkFrame(right, height=1, fg_color="#5C1A35")
        ...
        ctk.CTkLabel(status_frame,
                      ...
                      font=ctk.CTkFont(family="Consolas", size=10),
                      text_color="#7F8C8D").pack(side="left", padx=8)
    ...
    def _done(self):
        ...
        self.status_lbl.configure(text="● READY", text_color=ACCENT)
        self.table.refresh_from_db()
        self._log("[SYSTEM] All jobs finished.")

    def _stop(self):
        self.engine.stop()
        self.status_lbl.configure(text="● STOPPING...", text_color=ACCENT_HOVER)
